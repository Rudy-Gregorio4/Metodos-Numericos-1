...
def sor_method(A, b, X0, omega=1.1, tolerance=1e-3, max_iterations=100):
    """
    Resolución de un sistema de ecuaciones lineales usando el método SOR (Sobrerrelajación Sucesiva).
    ...
    for k in range(max_iterations):  # Iterar hasta el número máximo de iteraciones
        ...
        for i in range(n):  # Iterar sobre cada ecuación
            ...
            X_new[i] = (1 - omega) * X[i] + (omega * (b[i] - s1 - s2)) / A[i][i]  # Actualizar el valor de X_new[i] según la fórmula de SOR
...
if __name__ == "__main__":
    ...
    tolerance = 1e-3
    max_iterations = 100
    omega = 1.2  # Factor de relajación

    # Resolver el sistema utilizando el método SOR
    solution, iterations = sor_method(A, b, X0, omega, tolerance, max_iterations)
